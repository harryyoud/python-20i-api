...
class TwentyIRestAPI:
    def __init__(self, auth_url="https://auth-api.20i.com:3000", url="https://api.20i.com", auth=None):
        self.url = url.rstrip('/')

        if isinstance(auth, dict):
            # Use reseller token, subuser username and subuser password to limit to subuser scope
            if "bearer" in auth and "username" in auth and "password" in auth:
                headers = {"Authorization": "Bearer {}".format(auth["bearer"])}
                o = {"grant_type": "password", "username": auth["username"], "password": auth["password"]}
                r = requests.post(auth_url+"/login/authenticate", json=o, headers=headers)
                try:
                    token = self.token_to_bearer(r.json()["access_token"])
                    self.auth = token
                except ValueError as e:
                    raise Exception("Got unexpected response from login endpoint") from e
            # Use reseller token and subuser username to limit to subuser scope
            elif "bearer" in auth and "username" in auth:
                headers = {"Authorization": "Bearer {}".format(auth["bearer"])}
                r = []
                r.append(requests.get(auth_url+"/user/stack-user", headers=headers).json())
                r.append(requests.get(auth_url+"/user/service-user", headers=headers).json())
                username = None
                for user in r[0]:
    ...
    def decode_response(self, response):
        ...
        try:
            ...
            raise ValueError("No valid JSON was returned from the server") from e
    ...
    def post(self, endpoint=None, data=None, **kwargs):
        if endpoint is None:
            raise ValueError("no URL supplied")
        headers = {"Authorization": "Bearer {}".format(self.auth)}
        r = requests.post(self.get_url(endpoint), json=data, **kwargs, headers=headers)
        return self.decode_response(r)
